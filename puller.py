...
def srch():
  ...
  for folder in folders:
    ...
    gp=[]
    for file in files:
     if "game pass" in file.lower():
       gp.append(file)
    s2=open("gamepasses.txt", "a")
    for f in gp:
      with open(f"{folder}/{f}", "r") as s:
        lines = s.read()
        s2.write(lines)
        with open(f"{folder}/{f}", "w") as suh:
          suh.write("")
    s2.close()
...
class Utils:
    ...
    def remove_content(filename: str, delete_line: str) -> None:
        with thread_lock, open("gamepasses.txt", "r+") as file:
...
def main(ms_creds : str):
    ...
    if "resource" in response.json():
        ...
        if "ineligible" in response.text:
            Utils.remove_content('gamepasses.txt',ms_creds)
        return
    code = link.split("/")[-1]
    r = requests.get(f"https://discord.com/api/v9/entitlements/gift-codes/{code}?country_code=ES&with_application=false&with_subscription_plan=true")
    if r.status_code == 200 and r.json()["uses"] == 1:
      Utils.sprint(f'[-] Successfully fetched promo link -> {link} | Already Claimed','')
      Utils.remove_content('gamepasses.txt',ms_creds)
      return
    Utils.sprint(f'[+] Successfully fetched promo link -> {link} | Code: {r.status_code}','new')
    Utils.remove_content('gamepasses.txt',ms_creds)
...
if __name__ == "__main__":
  ...
  accs = list(set(open('gamepasses.txt').read().splitlines()))
  while len(accs) > 0:
    try:
      local_threads = []
